...
def test_rtc9810_tc_des_005_dcm_ports_profiles_packet_loss():
    """
    Introduce packet loss in the individual ports of MCE capturing a profile of video, trigger a recording and
    verify recording is incomplete or complete and verify if the rest of the profiles can be played back
    """
    ssh_client = None
    response = None
    ...
    try:
        ...
        for mce_node in mce_nodes:
            ...
            des_cmd = DestructiveTesting.PACKET_LOSS_PORT.format(DestructiveTesting.IFB_INTERFACE,
                                                                 DestructiveTesting.PACKET_LOSS, profile_port)
        ...
        is_valid, error = validate_recordings.validate_recording_end_state(
            response[RecordingAttribute.RECORDING_ID], recording_status,
            web_service_obj=response[RecordingAttribute.WEB_SERVICE_OBJECT], end_time=end_time)

        assert is_valid, error

        is_valid, bitrates = utils.get_video_profiles_from_m3u8(response[RecordingAttribute.RECORDING_ID])
        assert is_valid, bitrates
        # If Valid, bitrates will contain the list of video profiles
        assert bitrates, ValidationError.VIDEO_PROFILES_NOT_FOUND.format(response[RecordingAttribute.RECORDING_ID])
        ...
        if bitrates:
            for bitrate in bitrates:
                ...
                is_valid, playback_error = validate_recordings.validate_playback_using_vle(
                    response[RecordingAttribute.RECORDING_ID], VLE_REQUEST_PARAMS=vle_request_params)
                if not is_valid:
                    break
        else:
            is_valid = False
