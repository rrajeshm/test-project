...
def test_rtc9791_tc_rec_040_bulk_deletion_recon_assets(total_recording, stream):
    """
    Bulk deletion of assets in Recon(20 requests)
    """
    ...
    try:
        ...
        playback_pool = mp_pool.ThreadPool(processes=total_recording)
        ...
        for i in range(total_recording):
            ...
            is_valid, error = validate_storage.validate_recording_in_storage(response, Cos.RECON_STORAGE,
                                                                             [Cos.RECORDING_PARTIALLY_STORED, Cos.RECORDING_STORED])
            assert is_valid, error
