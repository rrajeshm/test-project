...
TC9786_DATA = [("rtc9786_bulk_update_end_time_before_recording_start_unique", RecordingAttribute.COPY_TYPE_UNIQUE)]


@utils.test_case_logger
@pytest.mark.parametrize("total_recording", TOTAL_RECORDINGS_LIST)
@pytest.mark.parametrize("name, copy_type", TC9786_DATA, ids=[x[0] for x in TC9786_DATA])
def test_rtc9786_bulk_update_end_time_before_recording_start_unique(total_recording, stream, name, copy_type):
    """
    TC9786 : Bulk update of recording requests of end time before recording(20 requests ) starts unique copy
    """
    recording = None
    web_service_objects = []
    recording_pool = None
