...
@pytest.mark.parametrize("name, copy_type", TC_REC_008_034_DATA, ids=[x[0] for x in
                                                                      TC_REC_008_034_DATA])
def test_tc_rec_008_034_(total_recording, stream, name, copy_type):
    ...
    try:
        queue = Queue.Queue()
        start_time = utils.get_formatted_time(constants.SECONDS * 20, TimeFormat.TIME_FORMAT_MS, stream)
