...
@pytest.mark.parametrize("name, copy_type", TC_REC_037_DATA, ids=[x[0] for x in
                                                                  TC_REC_037_DATA])
def test_rtc9769_tc_rec_037_(total_recording, stream, name, copy_type):
    ...
    try:
        ...
        for i in range(total_recording):
            is_valid, error = validate_recordings.validate_notification(web_service_objects[i],
                                                                        constants.RecordingStatus.STARTED)
            assert is_valid, error
        ...
        for i in range(total_recording):
            recording_pool.apply_async(validate_recordings.validate_notification,
                                           (web_service_objects[i], constants.RecordingStatus.COMPLETE, wait_time),callback=queue.put)
        for i in range(total_recording):
        ...
        for i in range(total_recording):
            response = rio.find_recording(recording_id_list[i]).json()
            is_valid, error = validate_recordings.validate_actual_time(response, recording.get_entry(i).EndTime,
