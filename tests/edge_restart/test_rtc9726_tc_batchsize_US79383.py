...
TC_ER_001_002_DATA = [("rtc9726_bulk_recordings_unique", RecordingAttribute.COPY_TYPE_UNIQUE)]

@utils.test_case_logger
@pytest.mark.parametrize("name, copy_type", TC_ER_001_002_DATA, ids=[x[0] for x in
                                                                        TC_ER_001_002_DATA])
def test_rtc9726_tc_change_batch_size_multiple(stream, name, copy_type):
    """
    ...
    try:
        ...
        recording = recording_model.Recording(total_recordings=same_start_time_recordings, StartTime=start_time, EndTime=end_time,
        ...
        for i in range(diff_start_time_recordings, same_start_time_recordings+diff_start_time_recordings):
            ...
            end_time = utils.get_formatted_time((constants.SECONDS * 90) + i, TimeFormat.TIME_FORMAT_MS, stream)
            rec_with_diff_time = recording_model.Recording(total_recordings=1, StartTime=start_time, EndTime=end_time,
                                                           copyType=copy_type, StreamId=stream)
            rec_with_diff_time.Entries[0].RecordingId = RecordingAttribute.RECORDING_ID_PREFIX + rec_with_diff_time.RequestId + '_'+str(i)
        ...
        for i in range(total_recording):
            recording_pool.apply_async(validate_recordings.validate_playback_using_vle,
                               (recording.get_entry(i).RecordingId,), dict(in_progress = True), callback=queue.put)
