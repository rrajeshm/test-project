...
TC_ER_015_DATA = [("common", RecordingAttribute.COPY_TYPE_COMMON), ("unique", RecordingAttribute.COPY_TYPE_UNIQUE)]


@utils.test_case_logger
@pytest.mark.parametrize("name, copy_type", TC_ER_015_DATA, ids=[x[0] for x in
                                                                        TC_ER_015_DATA])
def test_rtc9739_tc_er_015_vertical_grouping(stream, name, copy_type):
    """
    Vertical Grouping - Enable vertical grouping excluding some top bitrates in MA
    """
    ...
    try:
        ...
        profile_info = dict((int(port),int(bitrate)) for port,bitrate in profile_data.iteritems())
        max_bitrate = max(profile_info.values())
        ...
        is_valid, error = verify_vertical_grouping(recording_id, vg_bit_rate = str(max_bitrate))
        assert is_valid, error
